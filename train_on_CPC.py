...
from cropping_model import score_weighted_regression_loss, listwise_view_ranking_loss
from cropping_dataset import CPCDataset
...
def create_dataloader():
    assert cfg.training_set == 'CPC', cfg.training_set
    dataset = CPCDataset(only_human_images=cfg.only_human,
...
class Trainer:
    def __init__(self, model):
        ...
        self.eval_results = []
        self.best_results = {'human_iou': 0, 'human_disp': 1.,
                             'FCDB_iou': 0., 'FCDB_disp': 1.,
    ...
    def train(self):
        ...
        for batch_idx, batch_data in enumerate(self.train_loader):
            ...
            heat_map  = batch_data[2].to(device)
            crop_mask = batch_data[3].to(device)
            part_mask = batch_data[4].to(device)
            score     = batch_data[5].to(device)
            # width     = batch_data[4].to(device)
            # height    = batch_data[5].to(device)
            contain_human = (torch.count_nonzero(part_mask[0, 4]) > 0)

            # random_ID = list(range(0, rois.shape[1]))
            # random.shuffle(random_ID)
            # chosen_ID = random_ID[ : cfg.view_per_image]
            # rois = rois[:,chosen_ID]
            # crop_mask = crop_mask[:,chosen_ID]
            # score = score[:,chosen_ID]

            pre_patition, pred_heat_map, pred_score = self.model(im, rois, crop_mask, part_mask)
            score_loss = None
    ...
    def eval(self):
        human_iou, human_disp = evaluate_on_FCDB_and_FLMS(self.model, dataset='both', only_human=True)
        FCDB_iou, FCDB_disp = evaluate_on_FCDB_and_FLMS(self.model, dataset='FCDB', only_human=False)
        FLMS_iou, FLMS_disp = evaluate_on_FCDB_and_FLMS(self.model, dataset='FLMS', only_human=False)

        self.eval_results.append([self.epoch, human_iou, human_disp, FCDB_iou, FCDB_disp, FLMS_iou, FLMS_disp])
        epoch_result = {'human_iou': human_iou, 'human_disp': human_disp,
                        'FCDB_iou': FCDB_iou, 'FCDB_disp': FCDB_disp,
    ...
    def record_eval_results(self):
        ...
        header = ['epoch', 'human_iou', 'human_disp',
                  'FCDB_iou', 'FCDB_disp', 'FLMS_iou', 'FLMS_disp']
        rows = [header]
